...
<head>
<meta charset="UTF-8">
<title>Space Station - Wave & Boss Update</title>
<style>
    ...
    .ui { position:absolute; top:10px; right:10px; display:flex; flex-direction:column; gap:5px; z-index:10; width: 210px; background: rgba(0,0,0,0.85); padding: 10px; border-radius: 8px; border: 1px solid #4af; max-height: 90vh; overflow-y: auto; }
    ...
    #stats { font-size: 13px; margin-bottom: 8px; line-height: 1.4; border-bottom: 1px solid #333; padding-bottom: 5px; }
...
<div class="ui" id="mainUI">
    ...
    <div id="uiContent">
        <div id="stats">
            ...
            <div id="waveHighscoreDisplayUI" style="color: #4af; font-weight: bold;">Best Wave: 1</div>
        </div>
        <button onclick="togglePause()">Pause</button>
        <button onclick="restartGame()">Restart</button>
        
        <div id="weaponShop">
            <span class="section-title">Våpen (Bytt med 1-4)</span>
            <div class="wpn-group">
                ...
                <button id="upgradePistolBtn" onclick="upgradeWeapon('pistol')">Upg (500c)</button>
            </div>
            <div class="wpn-group">
                ...
                <button id="upgradeSMGBtn" onclick="upgradeWeapon('smg')">Upg (500c)</button>
            </div>
            <div class="wpn-group">
                ...
                <button id="upgradeShotgunBtn" onclick="upgradeWeapon('shotgun')">Upg (500c)</button>
            </div>
            <div class="wpn-group">
                <button id="arSelect" onclick="selectWeapon('ar')">AR</button>
                <button id="buyARBtn" onclick="buyWeapon('ar', 1200)">Kjøp (1200c)</button>
                <button id="upgradeARBtn" onclick="upgradeWeapon('ar')">Upg (500c)</button>
            </div>
        </div>
        <button onclick="resetGameData()" style="color:#f44; font-size:9px; border-color:#f44; margin-top:10px;">RESET ALL DATA</button>
    </div>
</div>
...
function updateUI() {
    ...
    ['pistol', 'smg', 'shotgun', 'ar'].forEach(t => {
        ...
        if(weaponLevels[t] >= 1) upgBtn.innerText = "Maxed";
    });
}
...
function spawnEnemy() {
    ...
    // BOSS LOGIKK: Trigger ved Wave 20 ELLER 100k Score
    if (currentWave === 20 || score >= 100000) {
        if (!boss) {
            boss = { x: 150, y: -100, w: 100, h: 80, hp: 600, maxHp: 600, speedX: 2.5, speedY: 0.6 };
        }
        return;
    }
    ...
    } else if (roll < 0.35 && score > 3000) { // Heavy
        enemies.push({x, y: -50, w: 45, h: 45, speedY: 1.2, color: '#800', hp: 6, maxHp: 6, isHeavy: true});
    } else { // Vanlig
        enemies.push({x, y: -40, w: 30, h: 30, speedY: 2.2 + (currentWave*0.15), color: '#f44', hp: 1});
    }
    
    enemiesInWave++;
    if (enemiesInWave > 12 + currentWave) { 
        currentWave++; 
        enemiesInWave = 0; 
        waveTimer = 150; 
...
function fire() {
    ...
    const dmg = conf.dmg;
    if (conf.type === "triple") {
        [-2.5, 0, 2.5].forEach(vx => bullets.push({x: player.x+15, y: player.y, vx, vy: -11, dmg}));
    } else {
        bullets.push({x: player.x+15, y: player.y, vx: 0, vy: -12, dmg});
    }
    shootCooldown = conf.cooldown[lvl];
}
...
function update() {
    ...
    // Boss Oppdatering
    if (boss) {
        if (boss.y < 60) boss.y += boss.speedY;
        boss.x += boss.speedX;
        ...
        bullets.forEach((b, bi) => {
            if (b.x < boss.x + boss.w && b.x + 5 > boss.x && b.y < boss.y + boss.h && b.y + 10 > boss.y) {
                ...
                bullets.splice(bi, 1);
                if (boss.hp <= 0) {
                    coins += 1500; score += 25000; boss = null; currentWave++; updateUI();
                }
            }
        });
        
        // Sjekk kollisjon med boss
        if(!player.dead && player.x < boss.x + boss.w && player.x + 35 > boss.x && player.y < boss.y + boss.h && player.y + 35 > boss.y) {
            player.dead = true;
    ...
    pickups.forEach((p, i) => {
        ...
        if (player.x < p.x+20 && player.x+35 > p.x && player.y < p.y+20 && player.y+35 > p.y) {
            if (p.t === 'm') magnetTimer = 500; else coins += 100;
            pickups.splice(i, 1); updateUI();
        }
        if (p.y > 600) pickups.splice(i, 1);
    });

    enemies.forEach((e, i) => {
        e.y += e.speedY;
        if (e.isSinus) e.x += Math.sin(e.y/25) * 4;
        ...
        bullets.forEach((b, bi) => {
            if (b.x < e.x+e.w && b.x+5 > e.x && b.y < e.y+e.h && b.y+10 > e.y) {
                ...
                if (e.hp <= 0) {
                    if (Math.random() < 0.06) pickups.push({x: e.x, y: e.y, t: Math.random()>0.5?'m':'g'});
...
function draw() {
    ...
    if (!player.dead) {
        ...
        ctx.fillText("CRASHED!", player.x - 10, player.y);
    }

    if (boss) {
        ctx.fillStyle = '#ff00ff'; ctx.fillRect(boss.x, boss.y, boss.w, boss.h); // Boss er lilla
        ctx.fillStyle = 'red'; ctx.fillRect(boss.x, boss.y - 12, boss.w, 6);
        ctx.fillStyle = '#00ff00'; ctx.fillRect(boss.x, boss.y - 12, boss.w * (boss.hp/boss.maxHp), 6);
        ctx.fillStyle = 'white'; ctx.font = 'bold 12px Arial';
        ctx.fillText("BOSS", boss.x + 30, boss.y - 18);
    }
    ...
    enemies.forEach(e => {
        ...
        if (e.isHeavy) {
            ctx.fillStyle='red'; ctx.fillRect(e.x, e.y-6, e.w, 4);
            ctx.fillStyle='lime'; ctx.fillRect(e.x, e.y-6, e.w*(e.hp/e.maxHp), 4);
        }
    });
    ...
    if (waveTimer > 0) {
        ...
        ctx.fillText(`WAVE ${currentWave}`, 140, 280);
        ctx.font='14px Arial'; ctx.fillText("KLARGJØR VÅPEN!", 145, 305);
    }
    if (magnetTimer > 0) {
        ctx.fillStyle='#f0f'; ctx.font='12px Arial'; ctx.fillText("MAGNET ACTIVE", 10, 65);
    }
    if (gameOver) { 
        ctx.fillStyle='rgba(0,0,0,0.7)'; ctx.fillRect(0,0,400,600);
        ctx.fillStyle='red'; ctx.font='40px Arial'; ctx.fillText("GAME OVER", 80, 300); 
        ctx.fillStyle='white'; ctx.font='20px Arial'; ctx.fillText("Trykk Restart i menyen", 100, 340);
    }
}
...
canvas.addEventListener("mousemove", e => {
    ...
    player.x = (e.clientX - rect.left) * (400/rect.width) - 17.5;
});

function togglePause() { paused = !paused; }
function restartGame() { init(); }
function resetGameData() { if(confirm("Vil du slette alle poeng og våpen?")) { localStorage.clear(); location.reload(); } }
